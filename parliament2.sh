...
if [[ ! -f "${illumina_bam}" ]] || [[ ! -f "${ref_fasta}" ]]; then
    if [[ "${dnanexus}" == "True" ]]; then
        ...
        >&2 echo "ERROR: An invalid (nonexistent) input file has been specified."
        exit 1
    fi
fi
...
if [[ "${run_breakdancer}" != "True" ]] && [[ "${run_breakseq}" != "True" ]] && [[ "${run_manta}" != "True" ]] && [[ "${run_cnvnator}" != "True" ]] && [[ "${run_lumpy}" != "True" ]] && [[ "${run_delly_deletion}" != "True" ]] && [[ "${run_delly_insertion}" != "True" ]] && [[ "${run_delly_inversion}" != "True" ]] && [[ "${run_delly_duplication}" != "True" ]]; then
    >&2 echo "WARNING: Did not detect any SV modules requested by the user through command-line flags."
    >&2 echo "Running with default SV modules: Breakdancer, Breakseq, Manta, CNVnator, Lumpy, and Delly Deletion"
...
>&2 echo "Set up and index BAM/CRAM"

# Check if BAM file has already been processed -- if so, continue
if [[ -f "done.txt" ]]; then
    >&2 echo "BAM file and index both exist in the mounted volume; continuing"
else
    # Allow for CRAM files
    if [[ "${extn}" == "cram" ]] || [[ "${extn}" == "CRAM" ]]; then
        >&2 echo "CRAM file input"
        ...
        >&2 echo "BAM file input, no index exists"
        ...
        >&2 echo "BAM file input, index exists"
        mv "${illumina_bam}" input.bam
...
if [[ "${run_breakseq}" == "True" || "${run_manta}" == "True" ]]; then
    >&2 echo "Launching jobs that cannot be parallelized by contig"
fi
...
if [[ "${run_breakseq}" == "True" ]]; then
    >&2 echo "BreakSeq"
    mkdir -p log_files/breakseq_logs/
    ...
    timeout 6h /home/dnanexus/breakseq2-2.2/scripts/run_breakseq2.py --reference ref.fa \
        ...
        --sample "${prefix}" 1> log_files/breakseq_logs/"${prefix}".breakseq.stdout.log 2> log_files/breakseq_logs/"${prefix}".breakseq.stderr.log &
fi

# MANTA
if [[ "${run_manta}" == "True" ]]; then
    >&2 echo "Manta"
    mkdir -p log_files/manta_logs/

    timeout 6h runManta 1> log_files/manta_logs/"${prefix}".manta.stdout.log 2> log_files/manta_logs/"${prefix}".manta.stderr.log &
fi
...
if [[ "${run_cnvnator}" == "True" ]] || [[ "${run_delly}" == "True" ]] || [[ "${run_breakdancer}" == "True" ]] || [[ "${run_lumpy}" == "True" ]]; then
    >&2 echo "Launching jobs parallelized by contig"
    mkdir -p log_files/breakdancer_logs/
    mkdir -p log_files/cnvnator_logs/
    mkdir -p log_files/delly_deletion_logs/
    mkdir -p log_files/delly_duplication_logs/
    mkdir -p log_files/delly_insertion_logs/
    mkdir -p log_files/delly_inversion_logs/
    mkdir -p log_files/lumpy_logs/
    mkdir -p log_files/sambamba_logs/

    while read -r contig; do
        if [[ $(samtools view input.bam "${contig}" | head -n 20 | wc -l) -ge 10 ]]; then
            >&2 echo "Running on contig ${contig}"
            count=$((count + 1))
            
            if [[ "${run_breakdancer}" == "True" ]]; then
                >&2 echo "Running Breakdancer for contig ${contig}"
                timeout 4h /breakdancer/cpp/breakdancer-max breakdancer.cfg input.bam -o "${contig}" > breakdancer-"${count}".ctx 2> log_files/breakdancer_logs/"${prefix}".breakdancer."${contig}".stderr.log &
            ...
            if [[ "$run_cnvnator" == "True" ]]; then
                >&2 echo "Running CNVnator for contig ${contig}"
                runCNVnator "${contig}" "${count}" 1> log_files/cnvnator_logs/"${prefix}".cnvnator."${contig}".stdout.log 2> log_files/cnvnator_logs/"${prefix}".cnvnator."${contig}".stderr.log &
            ...
            if [[ "${run_delly}" == "True" ]] || [[ "${run_lumpy}" == "True" ]]; then
                >&2 echo "Running sambamba view"
                timeout 2h sambamba view -h -f bam -t "$(nproc)" input.bam "${contig}" > chr."${count}".bam 2> log_files/sambamba_logs/"${prefix}".sambamba."${contig}".stderr.log
                >&2 echo "Running sambamba index"
                sambamba index -t "$(nproc)" chr."${count}".bam 1> log_files/sambamba_logs/"${prefix}".sambamba."${contig}".stdout.log 2> log_files/sambamba_logs/"${prefix}".sambamba."${contig}".stderr.log
                
                check_threads

                if [[ "${run_delly_deletion}" == "True" ]]; then  
                    >&2 echo "Running Delly (deletions) for contig $contig"
                    timeout 6h delly -t DEL -o "${count}".delly.deletion.vcf -g ref.fa chr."${count}".bam 1> log_files/delly_deletion_logs/"${prefix}".delly_deletion."${contig}".stdout.log 2> ./log_files/delly_deletion_logs/"${prefix}".delly_deletion."${contig}".stderr.log & 
                ...
                if [[ "${run_delly_inversion}" == "True" ]]; then 
                    >&2 echo "Running Delly (inversions) for contig $contig"
                    timeout 6h delly -t INV -o $count.delly.inversion.vcf -g ref.fa chr."${count}".bam 1> log_files/delly_inversion_logs/"${prefix}".delly_inversion."${contig}".stdout.log 2> log_files/delly_inversion_logs/"${prefix}".delly_inversion."${contig}".stderr.log & 
                ...
                if [[ "${run_delly_duplication}" == "True" ]]; then 
                    >&2 echo "Running Delly (duplications) for contig ${contig}"
                    timeout 6h delly -t DUP -o "${count}".delly.duplication.vcf -g ref.fa chr."${count}".bam 1> log_files/delly_duplication_logs/"${prefix}".delly_duplication.stdout.log 2> log_files/delly_duplication_logs/"${prefix}".delly_duplication.stderr.log & 
                ...
                if [[ "${run_delly_insertion}" == "True" ]]; then 
                    >&2 echo "Running Delly (insertions) for contig ${contig}"
                    timeout 6h delly -t INS -o "${count}".delly.insertion.vcf -g ref.fa chr."${count}".bam 1> log_files/delly_insertion_logs/"${prefix}".delly_insertion."${count}".stdout.log 2> log_files/delly_insertion_logs/"${prefix}".delly_insertion."${count}".stderr.log & 
                ...
                if [[ "${run_lumpy}" == "True" ]]; then
                    >&2 echo "Running Lumpy for contig ${contig}"
                    timeout 6h /home/dnanexus/lumpy-sv/bin/lumpyexpress -B chr."${count}".bam -o lumpy."${count}".vcf ${lumpy_exclude_string} -k 1> log_files/lumpy_logs/"${prefix}".lumpy."${count}".stdout.log 2> log_files/lumpy_logs/"${prefix}".lumpy."${count}".stderr.log & 
...
>&2 echo "Converting results to VCF format"
mkdir -p sv_caller_results/

(if [[ "${run_lumpy}" == "True" ]]; then
    >&2 echo "Convert Lumpy results to VCF format"
    ls -sh *.vcf
    python /convertHeader.py "${prefix}" "${lumpy_merge_command}" | vcf-sort -c | uniq > lumpy.vcf

    if [[ -f lumpy.vcf ]]; then
        cp lumpy.vcf sv_caller_results/"${prefix}".lumpy.vcf

        python /vcf2bedpe.py -i lumpy.vcf -o lumpy.gff
        python /Lumpy2merge.py lumpy.gff "${prefix}" 1.0
    else
        >&2 echo "No outputs of Lumpy found. Continuing."
    fi
fi) &

(if [[ "${run_manta}" == "True" ]]; then
    >&2 echo "Convert Manta results to VCF format"
    if [[ ! -f manta/results/variants/diploidSV.vcf.gz && ! -f manta/results/stats/alignmentStatsSummary.txt ]]; then
        >&2 echo "No outputs of Manta found. Continuing."
    else  
        cp manta/results/variants/diploidSV.vcf.gz sv_caller_results/"${prefix}".manta.diploidSV.vcf.gz
        mv manta/results/variants/diploidSV.vcf.gz .
        gunzip diploidSV.vcf.gz
        python /Manta2merge.py 1.0 diploidSV.vcf "${prefix}"

        cp manta/results/stats/alignmentStatsSummary.txt sv_caller_results/"${prefix}".manta.alignmentStatsSummary.txt
    fi

fi) &

(if [[ "${run_breakdancer}" == "True" ]] && [[ -n "${concat_breakdancer_cmd}" ]]; then
    >&2 echo "Convert Breakdancer results to VCF format"
    ...
    if [[ -f breakdancer.output ]]; then
        cp breakdancer.output sv_caller_results/"${prefix}".breakdancer.ctx
        ...
        cp breakdancer.vcf sv_caller_results/"${prefix}".breakdancer.vcf
    else
        >&2 echo "No outputs of Breakdancer found. Continuing."
    fi
fi) &

(if [[ "${run_cnvnator}" == "True" ]] && [[ -n "${concat_cnvnator_cmd}" ]]; then
    >&2 echo "Convert CNVnator results to VCF format"
    # cat contents of each file into output file: lack of quotes intentional
    ...
    if [[ -f cnvnator.output ]]; then
        ...
        cp cnvnator.vcf sv_caller_results/"${prefix}".cnvnator.vcf
        cp cnvnator.output sv_caller_results/"${prefix}".cnvnator.output
    else
        >&2 echo "No outputs of CNVnator found. Continuing."
    fi
fi) &

(if [[ "${run_breakseq}" == "True" ]]; then
    >&2 echo "Convert Breakseq results to VCF format"
    if [[ ! -f breakseq2/breakseq_genotyped.gff && ! -f breakseq2/breakseq.vcf.gz && ! -f breakseq2/final.bam ]]; then
        >&2 echo "No outputs of Breakseq found. Continuing."
    else
        mv breakseq2/breakseq.vcf.gz .
        gunzip breakseq.vcf.gz

        cp breakseq2/breakseq_genotyped.gff sv_caller_results/"${prefix}".breakseq.gff
        cp breakseq.vcf sv_caller_results/"${prefix}".breakseq.vcf
        cp breakseq2/final.bam sv_caller_results/"${prefix}".breakseq.bam
    fi
    ...
    if [[ -z $(find "${work}" -name "*.log") ]]; then
        >&2 echo "No Breakseq log files found."
    else
        cd "${work}" || return
        find ./*.log | tar -zcvf log.tar.gz -T -
        rm -rf ./*.log
        mv log.tar.gz log_files/breakseq_logs/"$prefix".breakseq.log.tar.gz
        cd ..
    fi
...
(if [[ "${run_delly_deletion}" == "True" ]]; then 
    >&2 echo "Convert Delly deletion results to VCF format"
    python /convertHeader.py "${prefix}" "${delly_deletion_concat}" | vcf-sort -c | uniq > delly.deletion.vcf

    if [[ -f delly.deletion.vcf ]]; then
        cp delly.deletion.vcf sv_caller_results/"${prefix}".delly.deletion.vcf
    else
        >&2 echo "No Delly deletion results found. Continuing."
    fi
fi) &

(if [[ "${run_delly_inversion}" == "True" ]]; then
    >&2 echo "Convert Delly inversion results to VCF format"
    python /convertHeader.py "${prefix}" "${delly_inversion_concat}" | vcf-sort -c | uniq > delly.inversion.vcf

    if [[ -f delly.inversion.vcf ]]; then
        cp delly.inversion.vcf sv_caller_results/"${prefix}".delly.inversion.vcf
    else
        >&2 echo "No Delly inversion results found. Continuing."
    fi
fi) &

(if [[ "${run_delly_duplication}" == "True" ]]; then
    >&2 echo "Convert Delly duplication results to VCF format"
    ...
    if [[ -f delly.duplication.vcf ]]; then
        cp delly.duplication.vcf sv_caller_results/"${prefix}".delly.duplication.vcf
    else
        >&2 echo "No Delly duplication results found. Continuing."
    fi
fi) &

(if [[ "${run_delly_insertion}" == "True" ]]; then
    >&2 echo "Convert Delly insertion results to VCF format"
    python /convertHeader.py "${prefix}" "${delly_insertion_concat}" | vcf-sort -c | uniq > delly.insertion.vcf

    if [[ -f delly.insertion.vcf ]]; then
        cp delly.insertion.vcf sv_caller_results/"${prefix}".delly.insertion.vcf
    else
        >&2 echo "No Delly insertion results found. Continuing."
    fi
fi) &
...
if [[ -z $(find . -name "*.vcf") ]]; then
    if [[ "${dnanexus}" == "True" ]]; then
        ...
        >&2 echo "ERROR: SVTyper requested, but candidate VCF files required to genotype. No VCF files found."
        exit 1
    fi
fi
set +e
...
if [[ "${run_genotype_candidates}" == "True" ]]; then
    ...
    >&2 echo "Running SVTyper"
    mkdir -p svtyped_vcfs/

    i=0
    # Breakdancer
    if [[ "${run_breakdancer}" == "True" ]]; then
        >&2 echo "Running SVTyper on Breakdancer outputs"
    ...
    # Breakseq
    if [[ "${run_breakseq}" == "True" ]]; then
        >&2 echo "Running SVTyper on BreakSeq outputs"
        ...
        if [[ -f breakseq.vcf ]]; then
            ...
            >&2 echo "No BreakSeq VCF file found. Continuing."
        fi
    fi

    # CNVnator
    if [[ "${run_cnvnator}" == "True" ]]; then
        >&2 echo "Running SVTyper on CNVnator outputs"
        ...
        if [[ -f cnvnator.vcf ]]; then
            ...
            >&2 echo "No CNVnator VCF file found. Continuing."
        fi
    fi

    # Delly
    if [[ "${run_delly}" == "True" ]]; then
        >&2 echo "Running SVTyper on Delly outputs"
        if [[ -z $(find . -name "delly*vcf") ]]; then
            >&2 echo "No Delly VCF file found. Continuing."
        else
    ...
    >&2 echo "Preparing inputs for SURVIVOR"
    for item in *svtyped.vcf; do
        python /adjust_svtyper_genotypes.py "${item}" > adjusted.vcf
        mv adjusted.vcf "${item}"
        >&2 echo "Adding ${item} to SURVIVOR inputs"
        >&2 echo "${item}" >> survivor_inputs
    done

    # Prepare SVtyped VCFs for upload
    for item in *svtyped.vcf; do
        cp "${item}" svtyped_vcfs/"${item}"
    done

    # Run SURVIVOR
    >&2 echo "Running SURVIVOR"
    ...
    python /combine_combined.py survivor_sorted.vcf "${prefix}" survivor_inputs /all.phred.txt | python /correct_max_position.py > "${prefix}".combined.genotyped.vcf
    cp survivor_sorted.vcf "${prefix}".survivor_sorted.vcf
    ...
    if [[ "${run_svviz}" == "True" ]]; then
        ...
        >&2 echo "Running svviz"
        mkdir -p log_files/svviz_logs/
        mkdir svviz_outputs

        grep \# survivor_sorted.vcf > header.txt

        if [[ "${svviz_only_validated_candidates}" == "True" ]]; then
            >&2 echo "Only visualizing validated candidates"
        ...
        if [[ "${vcf_entries}" == 0 ]]; then
            ...
            >&2 echo "No entries in the VCF to visualize. Not running svviz."
        else
            ...
            for item in small_vcf*; do
                ...
                >&2 echo "svviz --pair-min-mapq 30 --max-deletion-size 5000 --max-reads 10000 --fast --type batch --summary svviz_summary.tsv -b input.bam ref.fa survivor_split.${count}.vcf --export svviz_outputs" >> commands.txt
                ((count++))
            done
            ...
            #parallel --memfree 5G --retries 2 --verbose -a commands.txt eval 1>log_files/svviz_logs/svviz.stdout.log 2>log_files/svviz_logs/svviz.stderr.log
            parallel --retries 2 --verbose -a commands.txt eval 1>log_files/svviz_logs/svviz.stdout.log 2>log_files/svviz_logs/svviz.stderr.log
            
            cd svviz_outputs && tar -czf "${prefix}".svviz_outputs.tar.gz .
            cd ..
        fi
    fi
fi

cd log_files/ && find . -type d -empty -delete && find . -maxdepth 1 -mindepth 1 -type d -exec tar czf {}.tar.gz {} --remove-files \;
