...
samtools index indel_realigned."${contig}".bam

xatlas --ref "${ref_genome}" --in indel_realigned."${contig}".bam --prefix "${prefix}.${contig}" -s "${prefix}.${contig}" --gvcf --enable-strand-filter 1> /home/dnanexus/out/log_files/xatlas.${contig}.stdout.log 2> /home/dnanexus/out/log_files/xatlas.${contig}.stderr.log
